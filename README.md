...
- 使用最多 5 折交叉验证比较候选模型，并自动降低小样本任务的折数
- 生成模型排行榜，展示交叉验证均值、标准差和留出集指标
- 自动选择交叉验证表现最好的模型
- 记录工作流 Trace，展示每个阶段的执行状态和耗时
- 输出分类/回归评估指标
- 保存清洗后的数据、特征预览和预处理摘要
- 生成分类/回归评估图表
- 生成最佳模型特征重要性
...
当前版本会根据任务类型自动训练一组候选模型。分类任务默认以 `f1_macro` 为主指标，回归任务默认以 `r2` 为主指标。

每个候选模型会执行最多 5 折交叉验证，系统按交叉验证均值选择最佳模型，并在固定留出集上给出最终指标。交叉验证标准差用于观察模型稳定性；小样本或类别样本不足时，系统会自动降低折数。网页端的模型排行榜会同时展示：
...
其中 `trace` 字段会记录数据读取、数据画像、Agent 规划、EDA、模型训练、报告生成和结果保存等阶段的状态与耗时。网页端会按执行顺序展示这些信息，便于定位性能瓶颈和失败阶段。
